...
- **TDD (Red-Green-Refactor)**: Always write a failing test first (Red), then write the minimum production code to make it pass (Green), then refactor. Never introduce new behavior without a failing test driving it. No production code without a failing test — the codebase evolves organically.
...
- **Test naming — domain language only**: Test names must describe behaviors using domain language. Zero technical or implementation terms allowed. Use the `given_when_then` pattern where each segment reads as natural language describing the business scenario.
  - **Forbidden terms**: method names (`findById`, `create`, `update`, `patch`, `delete`, `save`, `getById`, `findAll`, `getEmployees`), technical verbs (`return`, `persist`, `construct`, `convert`, `map`, `handle`), validation jargon (`valid`, `invalid`, `null`, `blank`-as-prefix), HTTP/framework terms (status codes, `ProblemDetail`, `DTO`, `ResponseEntity`, `Mono`, `Request`, `Response`, `Header`), exception class names, data structure terms (`fields`, `partial`, `empty`).
  - **Given** describes the precondition in domain terms: `givenCorrectEmployeeInformation`, `givenExistingEmployee`, `givenTheEmployeeIsNotStored`, `givenAnEmployeeWithoutEmployeeType`, `givenBlankFirstName`, `givenOnlyFirstName`.
  - **When** describes the user/system action: `whenEmployeeIsAdded`, `whenEmployeeInformationIsChanged`, `whenEmployeeIsChanged`, `whenEmployeeIsRemoved`, `whenEmployeeIsLookedUp`, `whenAllEmployeesAreLookedUp`, `whenEmployeeIsViewed`, `whenTheProblemIsReported`.
  - **Then** describes the expected outcome: `thenProvidesTheEmployee`, `thenProvidesTheNewEmployee`, `thenListsAllEmployees`, `thenEmployeeIsNotFound`, `thenEmployeeIsNotAdded`, `thenEmployeeInformationIsNotChanged`, `thenEmployeeIsNotChanged`, `thenGivenAndAddedInformationMustBeTheSame`, `thenOnlyFirstNameShouldBeChanged`, `thenDescribesTheProblem`.
  - **Examples**: `givenCorrectEmployeeInformation_whenEmployeeIsAdded_thenProvidesTheNewEmployee`, `givenExistingEmployee_whenEmployeeInformationIsChanged_thenProvidesTheEmployee`, `givenEmployeeInformation_whenEmployeeIsAdded_thenGivenAndAddedInformationMustBeTheSame`.

...
- **Test naming — domain language only**: Test names must describe behaviors using domain language. No technical implementation details may leak into names. Forbidden: HTTP status codes (404, 201), technical types (ProblemDetail, DTO, ResponseEntity, Mono), framework terms (Request, Response, Header), exception class names. Good: `givenBlankFirstName_whenCreate_thenRejects`, `givenExistingEmployee_whenUpdate_thenReturnsSavedEmployee`.
